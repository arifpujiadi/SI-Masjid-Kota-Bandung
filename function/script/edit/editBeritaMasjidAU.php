...
if ($_POST['wilayahMasjid'] != '' &&
        $_POST['id_masjid'] != '' &&
        $_POST['judulBerita'] != '' &&
        $_POST['isiBerita'] != '') {
    $id_berita = $_REQUEST['id_berita_masjid'];
    $id_masjid = $_REQUEST['id_masjid'];
    ...
    if ($_SESSION['user'][2] == 0) {
        ...
        $nama_masjid = querySelect('*', 'masjid', 'id_user =' . $id_masjid, null);
        $jenis = "Masjid " . $nama_masjid[0]['nama_masjid'];
    }

    $update_berita = queryUpdate('berita_masjid', 'id_masjid="' . $id_masjid . '",jenis_admin="' . $jenis . '",judul="' . $judulBerita . '", isi_berita="' . $isiBerita . '", tanggal_upload="' . $date . '"', 'id_berita_masjid=' . $id_berita);
    ...
    $cariFoto = querySelect('*', 'berita_masjid', 'id_berita_masjid=' . $id_berita, null);
    ...
    if ($statusFoto == '' && $getFoto != '' || $statusFoto != '' && $getFoto != '') {
        ...
        if ((($_FILES["uploadgambarberita"]["type"] == "image/gif") || ($_FILES["uploadgambarberita"]["type"] == "image/jpeg") || ($_FILES["uploadgambarberita"]["type"] == "image/jpg") || ($_FILES["uploadgambarberita"]["type"] == "image/JPG") || ($_FILES["uploadgambarberita"]["type"] == "image/pjpeg") || ($_FILES["uploadgambarberita"]["type"] == "image/x-png") || ($_FILES["uploadgambarberita"]["type"] == "image/png"))
            ...
            if ($_FILES["uploadgambarberita"]["error"] > 0) {
                ...
                $mime = $mime[1];
                $imageName = $nama . "_" . $id_masjid . "_" . $id_berita . "." . $mime; 
    ...
    $cariFile = querySelect('*', 'berita_masjid', 'id_berita_masjid=' . $id_berita, null);
    ...
    if ($statusFile == '' && $getFile != '' || $statusFile != '' && $getFile != '') {
        if (isset($_FILES['uploadfile'])) {
            ...
            if (in_array($mime, array('pdf', 'zip')) && ($files['size'] >= 30000 && $files['size'] <= 8000000)) {
                ...
                $fileName = "FileBerita_" . $nama . "_" . $id_masjid . "_" . $id_berita . "." . $mime;
    ...
    $updateBerita = queryUpdate('berita_masjid', 'gambar="' . $gambarBerita . '",nama_file="' . $fileNameOri . '", alamat_file="' . $finalFile . '"', 'id_berita_masjid=' . $id_berita);
    var_dump($updateBerita);

    if ($update_berita) {
        header('location:http://localhost/SIMasjid/view/AdminUtama/isiAturBeritaMasjid.php?ubah=true&id_masjid=' . $id_masjid . '&id_berita_masjid=' . $id_berita . '&status=true');
    }
} else {
    header('location:http://localhost/SIMasjid/view/AdminUtama/isiAturBeritaMasjid.php?ubah=true&id_masjid=' . $id_masjid . '&id_berita_masjid=' . $id_berita . '&status=false');
}
?>