...
if ($_POST['wilayahMasjid'] != 'Pilih wilayah' &&
    ...
    if ($insert_pesanInbox && $insert_pesanSent) {
        header('location:http://localhost/SIMasjid/view/AdminUtama/tulis.php?status=true');
    }
    echo '<br>';
} else {
    header('location:http://localhost/SIMasjid/view/AdminUtama/tulis.php?&status=false');
}
?>